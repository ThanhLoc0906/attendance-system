...
class AttendanceApp:
    ...
    def create_student_screen(self):
        ...
        tk.Label(self.root, text="Student Dashboard",
                 font=("Times New Roman", 20, "bold"), fg="darkred", bg="white").pack(pady=10)
    ...
    def create_attendance_form(self):
        ...
        tk.Button(form_frame, text="Save Attendance",
                  ...
                  command=self.save_attendance).grid(row=0, column=6, padx=10, pady=5)
    ...
    def save_attendance(self):
        ...
        try:
            conn = sqlite3.connect(DB_FILE)
            c = conn.cursor()
            c.execute("INSERT INTO attendance (student id, student name, date, status) VALUES (?, ?, ?, ?)",
